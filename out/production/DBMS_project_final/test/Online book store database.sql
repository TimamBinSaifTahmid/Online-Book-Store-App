...
create or replace procedure retrieve_id(name1 in varchar2, password1 in varchar2, userId out number)
    is
...
begin
    open c for
select O_id,B_id,U_id,amount,price,Dt,due_Amount,paid_amount,Book.name,Users.name
         from Orders,Book,Users
         where Orders.B_id=Book.B_id and Orders.U_id=Users.U_id and due_Amount != 0;
end;
/
...
create or replace function panding_rating(c_id in Users.U_id%TYPE)
    return SYS_REFCURSOR
    is
    c SYS_REFCURSOR;
begin
    open c for
        select B_id,Book.name, quantity,price ,description,status,rating
        from Rating,
             Book
        where Rating.U_id = c_id
          and Rating.B_id = Book.B_id
          and pending = 1;
end;
/

create or replace procedure give_rate(Book_id in Book.B_id%type,user_id Users.U_id%type, new_rating in Book.rating%type)
    is
    prev_Rating number;
begin
  update Rating
  set pending=0
  where Rating.B_id=Book_id and Rating.U_id=user_id;
...
into cnt
from Users
where Name1=Users.name and Password1=Users.password and Users.U_type=0;
...
END IF;
 ...
 begin
     ...
     open c for
         select O_id,B_id,U_id,amount,price,Dt,due_Amount,paid_amount,Book.name,Users.name
         from Orders,Book,Users
         where Orders.B_id=Book.B_id and Orders.U_id=Users.U_id;
        return c;
 end;
 /
 ...
 create or replace procedure add_new_books(category_name in Book_catagory.Catagory_name%type,b_name in Book.name%TYPE,
										   ...
										   description in Book.description%type,status in Book.status%type,rating in Book.rating%type)
 is
...
create or replace function due_order_checker(Order_id in Oders.O_id%type)
return number
